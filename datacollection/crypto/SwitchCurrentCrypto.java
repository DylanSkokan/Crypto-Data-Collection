...
/**
 * <h2>SwitchRobot</h2> Performs a mock trade and switches values to accept new
 * crypto
 * 
 * @author Dylan Skokan
 * @since 2/23/22
 */
public class SwitchCurrentCrypto {
	public static void bitcoinSwitchRobot() {
		...
		SwingWorker<Void, Void> checkerRobot = new SwingWorker<Void, Void>() {
			...
			protected Void doInBackground() throws Exception {
				...
				// check if top option under "side" says buy or sell, if it says buy then sell
				// that coin, if it says sell
				...
				if (tradeType.equals("Buy")) {
					...
					// highlight under "market" to get name of coin (doesnt have to replace/remove
					// any chars)
					...
					fromCoin = (String) Toolkit.getDefaultToolkit().getSystemClipboard()
					...
					// Checks for buy/sell button (Possibly needs to check for something else if
					// limit is selected from start)
					waitingCounter = 0;
					...
					String testSpot = (String) Toolkit.getDefaultToolkit().getSystemClipboard()
							.getData(DataFlavor.stringFlavor);
					...
					for (int i = 0; i < 10; ++i) {
						...
						if (testSpot.equals(fromCoin)) {
							...
							waitingCounter = 0;
							robot.mouseMove(209, 737);
							while (robot.getPixelColor(209, 737).getRed() < 35
									&& robot.getPixelColor(209, 737).getGreen() < 35
									&& robot.getPixelColor(209, 737).getBlue() < 35) {
								robot.delay(200);
								waitingCounter++;
								if (waitingCounter >= 100) {
							...
							testSpot = (String) Toolkit.getDefaultToolkit().getSystemClipboard()
									.getData(DataFlavor.stringFlavor);
							testSpot = testSpot.replaceAll("-USD", "");
							testSpot = testSpot.replaceAll("\n", "");
						}
					...
					/*
					 * //click place order robot.mouseMove(138, 733); robot.delay(200);
					 * RF.click(robot); robot.setAutoDelay(vars.delayNum);
					 */
					...
					if (robot.getPixelColor(94, 778).getRed() > 230 && robot.getPixelColor(94, 778).getGreen() > 80
						...
						String SPrice = (String) Toolkit.getDefaultToolkit().getSystemClipboard()
								.getData(DataFlavor.stringFlavor);
						...
						/*
						 * //click place sell order robot.mouseMove(73, 728); robot.delay(200);
						 * RF.click(robot); robot.setAutoDelay(vars.delayNum);
						 */
				...
				if (vars.trySellBitcoin == false) {
					...
					Toolkit.getDefaultToolkit().getSystemClipboard().setContents(
							new StringSelection("https://pro.coinbase.com/trade/" + vars.currentTopBit + "-USD"), null);
					robot.setAutoDelay(vars.delayNum);
					...
					/*
					 * //click place buy order robot.mouseMove(138, 733); robot.delay(1000);
					 * RF.click(robot); robot.setAutoDelay(vars.delayNum);
					 */
					...
					if (robot.getPixelColor(94, 778).getRed() > 230 && robot.getPixelColor(94, 778).getGreen() > 80
						...
						/*
						 * //click place buy order robot.mouseMove(73, 728); robot.delay(200);
						 * RF.click(robot); robot.setAutoDelay(vars.delayNum);
						 */
			...
			protected void done() {
				...
				// vars.tryBuyBitcoin == true
					...
					PriceTracker.bitcoinTrackerRobot();
				} else {
					...
					for(BitcoinMethod currMethod : vars.methodResults) {
						currMethod.sold = false;
					}
					
					//put method results to pqueue for sorting
					var methodQueue = new PriorityQueue<BitcoinMethod>();
					methodQueue.addAll(vars.methodResults);
					
					//write success rates to file
					PrintWriter writer;
					try {
						writer = new PrintWriter("methodSuccess/methods.txt", "UTF-8");
						
						writer.println("Trade counter: " + vars.bitcoinSellCounter++ + "\n");
						
						while(!methodQueue.isEmpty()) {
							writer.println(methodQueue.poll());
						}
					...
					String logEntry = TimeManager.getCurrentTime() + "Switched from " + vars.currentlyWatchingBitcoin
							+ " to " + vars.currentTopBit + "\nSold for: " + vars.calcBitcoinChange + "%" + " at "
							+ vars.bitcoinPriceGlobal + "\nPC high point: " + vars.bitcoinPcHigh + "%" + " at "
							+ vars.highestBitcoinPrice + "\nPC low point: " + vars.bitcoinPcLow + "%" + " at "
							+ vars.lowestBitcoinPrice
							+ "\n------------------------------------";
					System.out.println(logEntry);
					System.out.println();
					GUI.bitcoinLogArea.append(logEntry + "\n");
