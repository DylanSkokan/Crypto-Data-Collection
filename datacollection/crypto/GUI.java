...
public class GUI extends JFrame implements ActionListener {
	...
	GUI() {
		...
		/*
		In its current state, the program tracks the stop gaining crypto in the past
		24 hours. It will sell the bitcoin and buy another bitcoin if it overtakes the top
		spot for long enough, but it will also test certain sell methods such as: selling if
		down 20% or gain 30%. The objective is to determine if there is a static "method" that
		can be used to gain money over time.
		
		This program also records the success rate of each method in the methodSuccess folder
		 */
		if(vars.methodResults.isEmpty()) {
			for(int i = 1; i < 150; i++) {
				for(int j = -100; j < -1; j++) {
					vars.methodResults.add(new BitcoinMethod(i, j));
				}
			}
		}

		//sets sysout statements to go to the log file instead
